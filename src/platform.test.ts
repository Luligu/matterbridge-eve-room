...
import { EveRoomPlatform } from './platform';
...
describe('TestPlatform', () => {
  ...
  it('should initialize platform with config name', () => {
    ...
    testPlatform = new EveRoomPlatform(mockMatterbridge, mockLog, mockConfig);
  ...
  it('should call onStart with reason', async () => {
    ...
    expect(testPlatform.room?.getAllClusterServers()).toHaveLength(9);
  });
  ...
  it('should execute the commandHandlers', async () => {
    expect(testPlatform.room).toBeDefined();
    expect(testPlatform.room?.getAllClusterServers()).toHaveLength(9);
    const identify = testPlatform.room?.getClusterServerById(Identify.Cluster.id);
